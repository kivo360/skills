...
class GlobalHooksDB:
    ...
    def environments_table(self):
        ...
        if self._environments_table is None:
            try:
                ...
                self._environments_table = self.db.create_table(
                    "environments", schema=EnvironmentConfig
                )
    ...
    def templates_table(self):
        ...
        if self._templates_table is None:
            try:
                ...
                self._templates_table = self.db.create_table(
                    "templates", schema=HookTemplate
                )
    ...
    def projects_table(self):
        ...
        if self._projects_table is None:
            try:
                ...
                self._projects_table = self.db.create_table(
                    "projects", schema=ScaffoldingProject
                )
    ...
    def analytics_table(self):
        ...
        if self._analytics_table is None:
            try:
                ...
                self._analytics_table = self.db.create_table(
                    "analytics", schema=HookUsageAnalytics
                )
    ...
    def search_hooks(self, query: str, limit: int = 10) -> list[HookMetadata]:
        ...
        """
        results = self.hooks_table.search(query).limit(limit).to_pydantic(HookMetadata)
        return results

    def get_hook_by_name(self, name: str) -> HookMetadata | None:
    ...
    def list_hooks(
        ...
        complexity: HookComplexity | None = None,
    ...
    def get_environment(self, name: str) -> EnvironmentConfig | None:
        ...
        """
        results = (
            self.environments_table.search()
            .where(f"name = '{name}'")
            .limit(1)
    ...
    def list_environments(
        self, env_type: Environment | None = None, active_only: bool = True
    ...
    def search_templates(self, query: str, limit: int = 10) -> list[HookTemplate]:
        ...
        """
        results = (
            self.templates_table.search(query).limit(limit).to_pydantic(HookTemplate)
        )
        return results

    def get_template_by_name(self, name: str) -> HookTemplate | None:
    ...
    def search_projects(self, query: str, limit: int = 10) -> list[ScaffoldingProject]:
        ...
        """
        results = (
            self.projects_table.search(query)
            .limit(limit)
            .to_pydantic(ScaffoldingProject)
        )
        return results

    def get_project_by_name(self, name: str) -> ScaffoldingProject | None:
        """Get project by name.
    ...
    def record_hook_usage(
        ...
        if (
            analytics.min_execution_time == 0.0
            or execution_time < analytics.min_execution_time
        ):
            analytics.min_execution_time = execution_time
        analytics.max_execution_time = max(analytics.max_execution_time, execution_time)

        # Calculate new average
        total_time = (
            analytics.avg_execution_time * (analytics.total_executions - 1)
            + execution_time
        )
    ...
    def get_hook_analytics(self, hook_name: str) -> HookUsageAnalytics | None:
    ...
    def suggest_similar_hooks(
        self, hook_name: str, limit: int = 5
    ) -> list[HookMetadata]:
    ...
    def get_popular_hooks(self, limit: int = 10) -> list[HookMetadata]:
        ...
        analytics_results = self.analytics_table.search().to_pydantic(
            HookUsageAnalytics
        )
