...
from quickhooks.db.models import Environment, HookComplexity, HookMetadata, HookType
...
class HookIndexer:
    ...
    def index_hook_file(self, file_path: Path) -> HookMetadata | None:
        ...
        try:
            ...
            metadata = self._extract_metadata(
                file_path, hook_class, hook_cls, content, tree
            )
            return metadata
    ...
    def _find_hook_class(self, tree: ast.AST) -> ast.ClassDef | None:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                for base in node.bases:
                    if (isinstance(base, ast.Name) and base.id == "BaseHook") or (
                        isinstance(base, ast.Attribute) and base.attr == "BaseHook"
                    ):
                        return node
        return None
    ...
    def _determine_hook_type(self, hook_cls: type, content: str) -> HookType:
        ...
        if any(
            word in content_lower for word in ["validate", "check", "verify", "ensure"]
        ):
            return HookType.VALIDATOR
        if any(
            word in content_lower
            for word in ["transform", "modify", "convert", "change"]
        ):
            return HookType.TRANSFORMER
        if any(
            word in content_lower for word in ["analyze", "parse", "extract", "detect"]
        ):
            return HookType.ANALYZER
        if any(
            word in content_lower
            for word in ["enhance", "improve", "optimize", "enrich"]
        ):
            return HookType.ENHANCER
        if any(word in content_lower for word in ["log", "record", "track", "monitor"]):
            return HookType.LOGGER

        return HookType.TRANSFORMER  # Default

    def _analyze_complexity(
        self, hook_class: ast.ClassDef, content: str
    ) -> HookComplexity:
        """Analyze the complexity level of the hook."""
        # Count methods
        methods = [
            node for node in hook_class.body if isinstance(node, ast.FunctionDef)
        ]
    ...
    def _find_test_file(self, hook_path: Path) -> Path | None:
    ...
    def _find_config_file(self, hook_path: Path) -> Path | None:
    ...
    def _determine_environments(
        self, hook_cls: type, content: str
    ) -> list[Environment]:
        ...
        if docstring:
            ...
            for sentence in sentences:
                if any(
                    ...
                    for word in ["use", "for", "when", "during"]
                ):
    ...
    def _estimate_test_coverage(self, test_path: Path | None) -> float:
    ...
    def _score_documentation(
        self, hook_class: ast.ClassDef, hook_cls: type, content: str
    ) -> float:
