...
global_app = cyclopts.App(help="Manage global hooks")
console = Console()
...
def add_to_path():
    ...
    if not global_dir.exists():
        console.print(
            "❌ Global hooks directory doesn't exist. Run 'quickhooks global setup' first.",
            style="red",
        )
        return
...
def import_hook(
    ...
    project_dir: Annotated[
        Path | None,
        Parameter("--project", alias="-p", help="Project directory (defaults to current)"),
    ] = None,
):
    ...
    ----------
    hook_name
        Name of the global hook to import
    project_dir
...
'''
    ...
    console.print(
        f"💡 You can now import this hook in your project as: from hooks.{hook_name}_global import {hook_name.title().replace('_', '')}Hook"
    )
