...
create_app = cyclopts.App(
    help="Create hooks, configs, and CLI commands using templates"
)
console = Console()
...
def hook(
    ...
    description: Annotated[
        str | None, Parameter("--description", alias="-d", help="Hook description")
    ] = None,
    hook_type: Annotated[
        str,
        Parameter("--type", alias="-t", help="Hook type (validator, transformer, analyzer)"),
    ] = "validator",
    base_class: Annotated[
        str, Parameter("--base", alias="-b", help="Base class to inherit from")
    ] = "BaseHook",
    output_dir: Annotated[
        str | None, Parameter("--output", alias="-o", help="Output directory for hook files")
    ] = None,
    create_test: Annotated[
        bool, Parameter("--test/--no-test", help="Create test file")
    ] = True,
    global_hook: Annotated[
        bool, Parameter("--global", alias="-g", help="Create hook in global directory")
    ] = False,
    ...
    output_dir
        Output directory for hook files
    create_test
        Create test file (default: True)
    global_hook
        Create hook in global directory (default: False)
    interactive
    ...
    hook_code = generator.generate_hook_class(
        hook_name=name, description=description, base_class=base_class
    )
...
def config(
    name: str,
    description: Annotated[
        str | None, Parameter("--description", alias="-d", help="Config description")
    ] = None,
    output_dir: Annotated[
        str, Parameter("--output", alias="-o", help="Output directory")
    ] = "config",
    global_config: Annotated[
        bool, Parameter("--global", alias="-g", help="Create config in global directory")
    ] = False,
    interactive: Annotated[
        bool, Parameter("--interactive", alias="-i", help="Interactive mode")
    ] = False,
    ...
    global_config
        Create config in global directory (default: False)
    interactive
        Interactive mode (default: False)
    """
...
def cli_command(
    name: str,
    description: Annotated[
        str | None, Parameter("--description", alias="-d", help="Command description")
    ] = None,
    output_dir: Annotated[
        str, Parameter("--output", alias="-o", help="Output directory")
    ] = "cli",
...
def list_global():
    ...
    if not global_dir.exists():
        ...
        console.print(
            "Run 'quickhooks create hook --global' to create your first global hook."
        )
        return
