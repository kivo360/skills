...
console = Console()
app = cyclopts.App(help="Manage QuickHooks features and optional dependencies")
...
def install_commands() -> None:
    ...
    if len(missing) > 1:
        ...
        if essential_features.intersection(set(missing)):
            console.print(
                "Essential features: pip install quickhooks[essential]", style="yellow"
            )
        ...
        if len(pro_features.intersection(set(missing))) >= 3:
            console.print(
                "Professional features: pip install quickhooks[pro]", style="yellow"
            )
...
def diagnose() -> None:
    ...
    if "ai" in [
        f for features_list in missing_by_category.values() for f in features_list
    ]:
        console.print("• For AI features: pip install quickhooks[ai]")

    if any(
        f in ["analytics"]
        for features_list in missing_by_category.values()
...
Example commands:
  ...
  quickhooks scaffold config --format json
    ...
    if demo_text:
        if features.has(feature):
            console.print(
                f"✅ {feature.title()} features are available!", style="green"
            )
        else:
            console.print(
                f"❌ {feature.title()} features are not available", style="red"
            )
            console.print(
                f"Install with: pip install quickhooks[{feature}]", style="cyan"
            )
